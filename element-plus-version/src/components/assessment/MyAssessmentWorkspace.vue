...
const qualitativeResults = computed(() => resultIndicators.filter((i) => i.nature !== '定性评分'))
</script>
    ...
    <template v-if="!selectedCycle">
      <div class="p-6 md:p-8 space-y-6 overflow-y-auto custom-scrollbar">
        <div class="animate-in fade-in slide-in-from-bottom-2 duration-500">
          <h2 class="text-2xl font-bold tracking-tight text-slate-900">我的考核目标与结果</h2>
        ...
        <div class="grid gap-4">
          <el-card
            ...
            :key="cycle.id"
            shadow="hover"
            class="custom-el-card border-slate-200 hover:border-blue-200 transition-all cursor-pointer group hover:shadow-md animate-in fade-in slide-in-from-bottom-4 duration-500"
            @click="selectedCycleId = cycle.id"
            body-class="!p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
          >
              ...
              <div class="min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <el-tag
                    ...
                    effect="plain"
                    round
                    class="!font-bold !bg-amber-50 !border-amber-200"
                  >
                    待确认目标
                  </el-tag>
                  <el-tag
                    ...
                    effect="plain"
                    round
                    class="!font-bold !bg-emerald-50 !border-emerald-200"
                  >
                    已结案
                  </el-tag>
                </div>
      ...
      <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="px-6 pt-6 flex items-center gap-3 animate-in fade-in duration-300">
          <el-button
            ...
            class="!text-slate-500 hover:!text-slate-900 !px-2 hover:!bg-slate-100 !rounded-lg"
          >
        ...
        <div class="p-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
          <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 animate-in slide-in-from-top-4 duration-500">
            ...
            >
              <el-button
                size="large"
                class="!w-full !bg-red-600 hover:!bg-red-700 !text-white !font-bold !text-base !px-8 !h-12 !rounded-xl !border-red-800"
          ...
          <!-- Tabs -->
          <el-tabs v-model="activeTab" class="custom-el-tabs animate-in fade-in duration-700 delay-150">
            <el-tab-pane name="goals">
              <template #label>
                <span class="flex items-center gap-2 font-bold px-4">
                  <Target class="h-4 w-4" /> 考核指标设定 (目标)
                </span>
              </template>
                ...
                <div class="space-y-6">
                  ...
                  <div class="grid gap-4">
                    <el-card
                      ...
                      :key="ind.id"
                      shadow="never"
                      class="custom-el-card !border-slate-200 overflow-hidden relative group hover:!border-blue-300 transition-all duration-300"
                      body-class="!p-0"
                    >
                      <div
                        :class="[
                      ...
                      />
                      <div class="p-6 pl-8 flex flex-col md:flex-row gap-8 items-center">
                        <div class="flex-1 space-y-4 w-full">
                          <div class="flex items-center gap-3">
                            <el-tag type="info" effect="plain" round class="!h-6 !w-6 !p-0 !text-center !font-bold !bg-slate-50 group-hover:!bg-blue-50 group-hover:!text-blue-600 transition-colors">
                              {{ index + 1 }}
                            </el-tag>
                            <h5 class="text-lg font-black text-slate-800 leading-none group-hover:text-blue-900">
                              {{ ind.name }}
                            </h5>
                            <el-tag type="info" effect="plain" round class="!font-medium !text-[10px] !border-none !bg-slate-100 !text-slate-500">
                              {{ ind.nature }}
                            </el-tag>
                          </div>
                        ...
                        <div class="w-full md:w-[260px] shrink-0 bg-blue-50/50 rounded-2xl border border-blue-100/50 p-5 relative overflow-hidden group-hover:bg-blue-50 group-hover:border-blue-200 transition-all">
                          <div class="flex justify-between items-center mb-4 relative z-10">
                            <span class="text-[10px] font-bold text-blue-800 uppercase tracking-widest opacity-60">考核基数</span>
                            <el-tag type="primary" effect="plain" round class="!bg-white/80 !shadow-sm !text-blue-700 !border-blue-200/50 !font-black">
                             权重 {{ ind.weight }}%
                            </el-tag>
                          </div>
              ...
              <div class="mt-4 space-y-8 pb-10">
                <div v-if="selectedCycle.status === 'finished'" class="space-y-10 animate-in fade-in duration-700">
                  ...
                  <div class="space-y-5">
                    ...
                    <div class="grid gap-4">
                      <el-card v-for="(ind, index) in quantitativeResults" :key="ind.id" shadow="never" class="!border-slate-200 overflow-hidden hover:shadow-lg transition-shadow duration-300 group custom-el-card">
                        <div class="flex items-center justify-between mb-6">
                          <div class="flex items-center gap-3">
                            ...
                            <el-tag type="info" effect="plain" class="!bg-slate-100 !text-slate-500 !font-bold !text-[10px] !h-5 !border-none">{{ ind.nature }}</el-tag>
                          </div>
                          <div class="text-right">
...
:deep(.custom-el-tabs .el-tabs__item) {
  font-size: 14px;
}
:deep(.custom-el-tabs .el-tabs__active-bar) {
  height: 3px;
  border-radius: 3px;
}
:deep(.custom-el-tabs .el-tabs__nav-wrap::after) {
  height: 1px;
